...
try:
    ...
    data = pandas.read_csv("flashcardapp/data/words_to_learn.csv")
except FileNotFoundError:
    # Use forward slashes for paths
    original_data = pandas.read_csv("flashcardapp/data/french_words.csv")
    data_dict = original_data.to_dict(orient="records")
except ValueError: # Keep ValueError in case words_to_learn.csv is empty or malformed
    # Use forward slashes for paths
    original_data = pandas.read_csv("flashcardapp/data/french_words.csv")
    data_dict = original_data.to_dict(orient="records")
else:
...
def is_known():
    ...
    # Use forward slashes for paths
    learn_data.to_csv("flashcardapp/data/words_to_learn.csv", index=False)
...
# Use forward slashes for paths
card_front_png = PhotoImage(file="flashcardapp/images/card_front.png")
# Use forward slashes for paths
card_back_png = PhotoImage(file="flashcardapp/images/card_back.png")
...
# Use forward slashes for paths
wrong_png = PhotoImage(file="flashcardapp/images/wrong.png")
wrong_button = Button(image=wrong_png, highlightthickness=0, command=next_card)
wrong_button.grid(column=0, row=1)

# Use forward slashes for paths
right_png = PhotoImage(file="flashcardapp/images/right.png")
